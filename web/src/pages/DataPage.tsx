...
export function DataPage() {
  ...
  return (
    <Container size="xl" className="page-section"><Stack gap="xl">
      <div><Text className="editorial-kicker">Data catalog</Text><Title className="display-title">Every release, dimension, and row remains inspectable.</Title><Text className="lede">Data is normalized for exploration but source provenance is never discarded. Use CSV when you need the full filtered result instead of a browser page.</Text>{catalog.data && <Stack gap={4} mt="sm"><Group gap="xs"><Text size="xs" c="dimmed">Resolved snapshot: {catalog.data.meta.snapshot.id}</Text><Button variant="default" size="compact-sm" loading={resolvingLatest} onClick={() => { void resolveLatest(); }}>View latest</Button></Group>{latestError && <Text c="red" size="xs">The latest snapshot could not be resolved; the current pin remains.</Text>}</Stack>}</div>
